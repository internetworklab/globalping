...
export class LineTokenizer extends TransformStream<string, TokenObject> {
  constructor() {
    let buffer = "";
...
export class JSONLineDecoder extends TransformStream<TokenObject, unknown> {
  constructor() {
    super({
      transform(
        chunk: TokenObject,
...
export function makeRealDNSResponseStream(
  ...
  for (const target of targets) {
    urlParams.append("dnsTarget", JSON.stringify(target));
  }
  urlParams.set("l7PacketType", "dns");

  const headers = new Headers();
  headers.set("Content-Type", "application/json");

  return fetch(`${getApiEndpoint()}/ping?${urlParams.toString()}`)
    .then((res) => res.body)
    .then(
      (rawStream) =>
        rawStream
